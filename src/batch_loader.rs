        ...
        for entry in entries {
            ...
            let handle = tokio::spawn(async move {
                ...
                let tmdb_result = if let Some(year) = entry_clone.movie.year {
                    match tmdb_client.search_movie_with_year(&cleaned_title, Some(year)).await {
                        Ok(Some(movie)) => Some(movie),
                        Ok(None) => {
                            // Fallback without year
                            tmdb_client.search_movie_with_year(&cleaned_title, None).await.ok().flatten()
                        },
                        Err(_) => {
                            tmdb_client.search_movie_with_year(&cleaned_title, None).await.ok().flatten()
                        }
                ...
                let poster_url = tmdb_result.as_ref().and_then(|movie| movie.get_full_poster_url());
